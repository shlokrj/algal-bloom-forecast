#!/usr/bin/env python3
"""Run one-factor linear feature-group ablations on the held-out-year splits."""
...
def _write_metrics(path: Path, records: list[dict[str, Any]]) -> None:
    fields = ["feature_group", "split", "forecast_horizon_days", "model", "n", "mae", "rmse"]
...
def run(*, split_path: Path | None = None, feature_path: Path | None = None) -> Path:
    ...
    groups = build_feature_groups(split_records)
    ...
    for group_name, feature_names in groups.items():
        baseline_predictions = build_baseline_predictions(
            ...
            evaluation_records,
            history_records,
            linear_features=feature_names,
        )
        group_metrics = evaluate_baseline_predictions(
            baseline_predictions,
            model_names=("linear",),
        )
        metrics.extend({"feature_group": group_name, **metric} for metric in group_metrics)
        predictions.extend(
            {
    ...
    metrics_path = ROOT / "results/tables" / f"algal_bloom_feature_ablation_metrics_{run_id}.csv"
    predictions_path = (
        ROOT / "results/tables" / f"algal_bloom_feature_ablation_predictions_{run_id}.csv"
    )
    _write_metrics(metrics_path, metrics)
    _write_predictions(predictions_path, predictions)
    manifest = {
        "source_id": "algal_bloom_feature_ablation_results",
        ...
        "fit_policy": "one linear model per horizon and feature group, fitted on train rows only",
        "metrics": metrics,
    }
    manifest_path = ROOT / "data/manifests" / f"algal_bloom_feature_ablation_{run_id}.json"
    if manifest_path.exists():
        raise FileExistsError(f"refusing to overwrite immutable manifest: {manifest_path}")
    manifest_path.write_text(
        json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print(f"wrote {len(metrics)} ablation metric rows")
