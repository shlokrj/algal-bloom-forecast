...
def test_normalization_contract_records_units_time_and_quality_policies() -> None:
    ...
    assert "2=not evaluated" in contract["glerl_continuous"]["quality_policy"]
    assert contract["glerl_continuous"]["quality_flag_mapping"]["3"] == "suspect"
    assert "nonnumeric tokens" in contract["glerl_continuous"]["quality_flag_unmapped_policy"]
    ...
    assert "curated annual reference" in contract["target"]["unit_evidence"]
    assert contract["target"]["curated_annual_reference"].endswith("Annual_CI.xlsx")
